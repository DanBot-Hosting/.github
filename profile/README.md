# DanBot Hosting LTD <img src="https://cdn.discordapp.com/icons/639477525927690240/cb186c5eda22b335848b613d0a5d489b.webp" alt="Logo" width="40" align="right" />

Welcome to the **DanBot Hosting LTD** official GitHub organization!

We provide both **free** and **paid** hosting solutions, including:
- 🌐 Website Hosting
- 🤖 Bot Hosting
- 🖥️ VPS Hosting
- 🗄️ Dedicated Servers

We’re passionate about making hosting accessible to everyone — whether you’re running a personal project, building a business, or deploying something big, we’ve got you covered.

---

## 📂 Open Source Projects

Here are some of the projects we maintain:

- [**Pterodactyl Eggs**](https://github.com/DanBot-Hosting/pterodactyl-eggs)  
  🥚 Custom-built Pterodactyl eggs tailored for DBH.

- [**DBH Main Bot**](https://github.com/DanBot-Hosting/DanBotHostingStats)  
  🤖 Discord bot used to manage users and servers at DBH.

- [**DBH Beta Bot**](https://github.com/DanBot-Hosting/discord-bot)  
  🧪 A newer Discord bot written in TypeScript with Slash Commands for various utilities.

- [**DBH Docs**](https://github.com/DanBot-Hosting/Documentation-Site)  
  📖 Open-source documentation site for DBH.

- [**DBH Daemon**](https://github.com/DanBot-Hosting/DBH-Daemon)  
  🔧 Background daemon for nodes, enabling REST API calls to execute server functions.

- [**DBH Convoy Panel Bot**](https://github.com/DanBot-Hosting/Convoy-Panel-Bot)  
  🛠️ Discord bot designed to help staff manage VPS accounts.

---

## 🌟 About Us

DanBot Hosting started with a mission to make hosting affordable, reliable, and user-friendly.  
We serve thousands of users worldwide, offering scalable solutions to fit every need.

Visit us at 👉 [**danbot.host**](https://danbot.host)

---

> © DanBot Hosting LTD — All rights reserved.
